...
const ANTELOPE_SENDER_NAME = "mysender";
...
describe("TokenBridge Contract", function () {
    ...
    describe(":: Bridge to Antelope", async function () {
        ...
        it("Should let any sender request bridging of a registered ERC20Bridgeable token to Antelope" , async function () {
            expect(await evm_bridge.connect(user).bridge(token.address, HALF_TLOS, ANTELOPE_SENDER_NAME, "", {value: HALF_TLOS})).to.emit('BridgeToAntelopeRequested');
        ...
        it("Should not let senders request bridging of an unregistered token to Antelope" , async function () {
            ...
            await expect(evm_bridge.connect(user).bridge(token2.address, HALF_TLOS, ANTELOPE_SENDER_NAME, "", {value: HALF_TLOS})).to.be.revertedWith('Pair not found');
        });
        it("Should not let senders request bridging if minimum amount isn't reached" , async function () {
            await expect(evm_bridge.connect(user).bridge(token.address, ethers.utils.parseEther('0.1'), ANTELOPE_SENDER_NAME, "", {value: HALF_TLOS})).to.be.revertedWith('Minimum amount is not reached');
        });
        it("Should not let senders request bridging if fee isn't sent" , async function () {
            await expect(evm_bridge.connect(user).bridge(token.address, ONE_TLOS, ANTELOPE_SENDER_NAME, "", {value: ethers.utils.parseEther('0.1')})).to.be.revertedWith('Needs TLOS fee passed');
        });
        it("Should not let senders request bridging of a paused registered ERC20Bridgeable" , async function () {
            ...
            await expect(evm_bridge.connect(user).bridge(token.address, ONE_TLOS, ANTELOPE_SENDER_NAME, "", {value: HALF_TLOS})).to.be.revertedWith('Bridging is paused for token');
        });
        it("Should not let senders request bridging of a registered ERC20Bridgeable if allowance is too low" , async function () {
            await expect(evm_bridge.connect(user).bridge(token.address, ONE_TLOS.mul(2), ANTELOPE_SENDER_NAME, "", {value: HALF_TLOS})).to.be.revertedWith('Allowance is too low');
        });
        it("Should not let senders request more than " + MAX_REQUESTS + " requests" , async function () {
            ...
            // Add tokens to user (fake bridging from Antelope)
            expect(await evm_bridge.connect(antelope_bridge).bridgeTo(token.address, user.address, ONE_TLOS.mul(MAX_REQUESTS + 1), ANTELOPE_SENDER_NAME)).to.emit('Transfer');
            for(var i = 0; i < MAX_REQUESTS; i++){
                expect(await evm_bridge.connect(user).bridge(token.address, ONE_TLOS, ANTELOPE_SENDER_NAME, "", {value: HALF_TLOS})).to.emit('BridgeToAntelopeRequested');
            }
            await expect(evm_bridge.connect(user).bridge(token.address, ONE_TLOS, ANTELOPE_SENDER_NAME, "", {value: HALF_TLOS})).to.be.reverted;
        });
    ...
    describe(":: Bridge from Antelope", async function () {
        ...
        it("Should not let random addresses mint & send a registered ERC20Bridgeable token" , async function () {
            expect(await register.addPair(token.address, ANTELOPE_DECIMALS, ANTELOPE_ACCOUNT_NAME, TOKEN_NAME)).to.emit("PairAdded");
            await expect(evm_bridge.connect(user).bridgeTo(token.address, user.address, ONE_TLOS, ANTELOPE_SENDER_NAME)).to.be.revertedWith('Only the Antelope bridge EVM address can trigger this method !');
        });
        // WIP ============================================================================================================== >
